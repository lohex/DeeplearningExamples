"""Reproducible BBBC021 patch extraction with group-aware splits."""
...
class PatchExtractionConfig:
    ...
    work_dir: Path = Path("bbbc021_5class")
    patch_size: int = 224
...
def load_selected_metadata(
    ...
    """Download metadata and select the five target compounds."""
    metadata_path = _download(
        urljoin(config.base_url, "BBBC021_v1_image.csv"),
        paths.work / "BBBC021_v1_image.csv",
    )
    metadata = pd.read_csv(metadata_path)
    available = {str(value).casefold(): str(value) for value in metadata[COMPOUND].dropna().unique()}
...
def assign_splits(
    ...
    """Assign complete plate-well groups to train, validation or test."""
    ...
    for class_name, group in metadata.groupby("class_name", sort=True):
        ...
        if training_count + validation_count >= len(group_ids):
            training_count, validation_count = len(group_ids) - 2, 1
        mapping = {value: "train" for value in group_ids[:training_count]}
        mapping.update(
            {value: "validation" for value in group_ids[training_count : training_count + validation_count]}
        )
...
def source_summary(metadata: pd.DataFrame) -> pd.DataFrame:
    return (
        ...
        .agg(fovs=("ImageNumber", "size"), wells=("group_id", "nunique"), plates=(PLATE, "nunique"))
        .reset_index()
    )
...
def download_source_images(
    ...
    """Download required plate archives and extract only selected TIFF files."""
    plates = sorted(metadata[PLATE].astype(str).unique())
    manifest = pd.DataFrame({"plate": plates})
    manifest["archive"] = manifest["plate"].map(lambda plate: f"BBBC021_v1_images_{plate}.zip")
    manifest["url"] = manifest["archive"].map(lambda name: urljoin(config.base_url, name))
    manifest["bytes"] = [_remote_size(url) for url in tqdm(manifest["url"], desc="Inspecting archives")]
    ...
    needed = {str(value) for _, filename_column in CHANNELS.values() for value in metadata[filename_column].dropna()}
    found: set[str] = set()
    ...
    if missing:
        raise FileNotFoundError(f"Missing {len(missing)} TIFFs, examples: {sorted(missing)[:10]}")
    return manifest
...
def pilot_storage_estimate(
    ...
    """Estimate final storage from randomly sampled lossless PNG patches."""
    rng = np.random.default_rng(config.seed)
    ...
    for _, row in tqdm(sampled.iterrows(), total=len(sampled), desc="Pilot patches"):
        patch, _, _ = _random_patch(_load_fov(row, config, paths), rng, config.patch_size)
        encoded_sizes.append(len(_encode_png(patch)))
    raw_uint8 = config.total_patches * config.patch_size**2 * 3 / 2**30
    return pd.DataFrame(
        {
            "representation": ["PNG pilot estimate", "raw uint8", "raw uint16", "raw float32"],
            "GiB": [np.mean(encoded_sizes) * config.total_patches / 2**30, raw_uint8, 2 * raw_uint8, 4 * raw_uint8],
        }
    )
...
def generate_dataset(
    ...
    """Generate balanced patches and write WebDataset TAR shards."""
    ...
    with _ShardWriter(paths.output, config.shard_size) as writer:
        for class_name in sorted(TARGETS.values()):
            for split in ("train", "validation", "test"):
                group = metadata[(metadata["class_name"] == class_name) & (metadata["split"] == split)]
                patch_count = int(
                    allocation[(allocation["class_name"] == class_name) & (allocation["split"] == split)]["patches"].iloc[0]
                )
                _generate_group(group, patch_count, class_name, split, writer, records, rng, config, paths)
    manifest = pd.DataFrame(records)
    manifest.to_csv(paths.output / "manifest.csv", index=False)
    (paths.output / "config.json").write_text(json.dumps(asdict(config), indent=2, default=str))
    return manifest


def validate_manifest(manifest: pd.DataFrame, config: PatchExtractionConfig) -> pd.DataFrame:
    if len(manifest) != config.total_patches:
        raise ValueError(f"Expected {config.total_patches} patches, received {len(manifest)}.")
    ...
    return (
        ...
        .agg(
            ...
            patches_per_fov=("source_fov_id", lambda values: len(values) / values.nunique()),
        )
        .reset_index()
    )
...
def _normalize_channel(channel: NDArray[np.generic], config: PatchExtractionConfig) -> NDArray[np.uint8]:
    ...
    return np.round(255 * np.clip((values - low) / (high - low), 0, 1)).astype(np.uint8)


def _load_fov(row: pd.Series, config: PatchExtractionConfig, paths: PipelinePaths) -> NDArray[np.uint8]:
    channels = []
...
def _random_patch(image: NDArray[np.uint8], rng: np.random.Generator, patch_size: int) -> tuple[NDArray[np.uint8], int, int]:
...
def _generate_group(
    ...
    while accepted < count and attempts < 20 * count:
        ...
        if source_id not in cache:
            ...
            used[source_id] = set()
        patch, y, x = _random_patch(cache[source_id], rng, config.patch_size)
        attempts += 1
        if (y, x) in used[source_id]:
            continue
        used[source_id].add((y, x))
        if float((np.max(patch, axis=-1) >= 8).mean()) < config.minimum_foreground_fraction:
            continue
    ...
    progress.close()
    if accepted < count:
        raise RuntimeError(f"Only generated {accepted}/{count} for {class_name}/{split}.")
...
class _ShardWriter:
    ...
    def _open_next_shard(self) -> None:
        self.close()
        self._shard_index += 1
        path = self.output_dir / f"bbbc021-{self._shard_index:05d}.tar"
